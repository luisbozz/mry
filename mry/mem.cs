...
namespace mry
{
    ...
    public class mem
    {
        ...
        public IntPtr rip(IntPtr p)
        {
            byte[] buffer = new byte[16];
            ReadProcessMemory(procHandle, p, buffer, buffer.Length, IntPtr.Zero);
            return IntPtr.Add(p, (BitConverter.ToInt32(buffer, 0) + 4));
        }
        ...
        {
            return await new AOBScan((uint)pid).AobScanBoyerMoore(data);
        }

        public async Task<IEnumerable<UIntPtr>> AOBScanTest(string aob)
        {
            return await new AOBScan((uint)pid).AobScanTest(aob);
        }

        public Pointer AOBScanModule(string data, ProcessModule processModule)
        {
            return new AOBScan((uint)pid).AobScanModule(data, processModule);
        }

        public Pointer AOBScanModule2(string data, ProcessModule processModule)
        {
        ...
        public class Memory
        {
            ...
            public unsafe byte[] GetBytes(long length)
            {
                byte[] buffer = new byte[length];
            ...
            public bool SetShort(string value)
            {
                if (isProcOpen)
                {
                    if (value != "")
                    {
                        ...
                        try
                        {
                            bytes = BitConverter.GetBytes(short.Parse(value));
                        }
                        catch (Exception)
                        {
                            bytes = BitConverter.GetBytes(ushort.Parse(value));
                        }
        ...
        {
            private const int OFFSET_ARG_COUNT = 8;

            private static IntPtr handle;
            private static ProcessModule gtaModule;
            private static long natives_registration_address;
            private static long get_native_handler_address;
            private static IntPtr return_stack;
            private static IntPtr arg_stack;
            private static IntPtr native_call_context;
            private static Array entrypoints;

            public void init(IntPtr proc)
            {
                handle = proc;
                long time = 0;
                SigScanSharp s = new SigScanSharp(proc);
                gtaModule = Process.GetProcessesByName("gta5")[0].MainModule;
                s.SelectModule(gtaModule);
                Pointer nativepointer = s.FindPattern("48 8D 0D ? ? ? ? 48 8B 14 FA E8 ? ? ? ? 48 85 C0 75 0A", out time);
                natives_registration_address = rip(IntPtr.Add((IntPtr)nativepointer, 3), 8).ToInt64();
                get_native_handler_address = rip(IntPtr.Add((IntPtr)nativepointer, 12), 8).ToInt64();
                IntPtr hglobal = Marshal.AllocHGlobal(100);
                //IntPtr oldProtection;
                //Win32.VirtualProtectEx(procHandle, gtaModule.BaseAddress, 8 * 10, Win32.MemoryProtection.ExecuteReadWrite, out oldProtection);
                //Win32.VirtualAllocEx(handle, gtaModule.BaseAddress, 8*10, Win32.AllocationType.Commit, Win32.MemoryProtection.)
                //Win32.VirtualProtectEx(procHandle, gtaModule.BaseAddress, 8 * 10, oldProtection, out oldProtection);

            }

            private IntPtr rip(IntPtr p, int bufferSize)
            {
                byte[] buffer = new byte[bufferSize];
                Win32.ReadProcessMemory(handle, p, buffer, (ulong)bufferSize);
                return IntPtr.Add(p, (BitConverter.ToInt32(buffer, 0) + 4));
            }
            ...
            {
                [DllImport("kernel32.dll")]
                public static extern bool ReadProcessMemory(IntPtr hProcess, IntPtr lpBaseAddress, byte[] lpBuffer, ulong dwSize, int lpNumberOfBytesRead = 0);
                [DllImport("kernel32.dll", SetLastError = true, ExactSpelling = true)]
                public static extern IntPtr VirtualAllocEx(IntPtr hProcess, IntPtr lpAddress, uint dwSize, AllocationType flAllocationType, MemoryProtection flProtect);
                [DllImport("kernel32.dll")]
                public static extern bool VirtualProtectEx(IntPtr hProcess, IntPtr lpAddress, uint dwSize, MemoryProtection flNewProtect, out IntPtr lpflOldProtect);
        ...
        public class AOBScan
        {
            ...
            public AOBScan(uint ProcessID)
            {
                ...
                this.proc = Process.GetProcessById((int)ProcessID);
            }
            ...
            public async Task<List<UIntPtr>> AobScan(string AOB)
            {
                Pattern.Byte[] pattern = Pattern.Transform(AOB);

                if (proc.Id == 0) return new List<UIntPtr>(new UIntPtr[] { UIntPtr.Zero });
                    ...
                    foreach (List<MEMORY_BASIC_INFORMATION64> ChunkOfHundred in ListOfChunks)
                    {
                        new Thread(new ThreadStart(() =>
                        {
                            Parallel.ForEach(ChunkOfHundred, (item, loopstate) =>
                            {
                                ...
                                ReadProcessMemory(proc.Handle, item.BaseAddress, buff, item.RegionSize, 0);
            ...
            public async Task<List<UIntPtr>> AobScanBoyerMoore(byte[] AOB)
            {
                if (proc.Id == 0) return new List<UIntPtr>(new UIntPtr[] { UIntPtr.Zero });
                MemoryRegion = new List<MEMORY_BASIC_INFORMATION64>();
                MemInfo(proc.Handle);
            ...
            public async Task<List<UIntPtr>> AobScanTest(string AOB)
            {
                if (proc.Id == 0) return new List<UIntPtr>(new UIntPtr[] { UIntPtr.Zero });
                MemoryRegion = new List<MEMORY_BASIC_INFORMATION64>();
                MemInfo(proc.Handle);
                    ...
                    foreach (List<MEMORY_BASIC_INFORMATION64> ChunkOfHundred in ListOfChunks)
                    {
                        new Thread(new ThreadStart(() =>
                        {
                            Parallel.ForEach(ChunkOfHundred, (item, loopstate) =>
                            {
                                ...
                                if (temp)
                                {
                                    resultCollection.Add((UIntPtr)((ulong)item.BaseAddress + (ulong)Result));
                                }
                            });
                            a++;
                        }), 1000).Start();
            ...
            public Pointer AobScanModule(string AOB, ProcessModule processModule)
            {
                SigScanSharp s = new SigScanSharp(proc.Handle);
                s.SelectModule(processModule);
                Pointer nativepointer = s.FindPattern(AOB, out _);

                return nativepointer;
            }

            public Pointer AobScanModule2(string AOB, ProcessModule processModule)
            {
                AOB = AOB.ToLower().Replace("??", "?").Replace("xx", "x");
                string[] bPatterns = AOB.ToLower().Split(' ');
                byte[] bPattern = new byte[bPatterns.Length];
                ...
                try
                {
                    buff = new byte[item.ModuleMemorySize];
                }
                catch (Exception)
                {
                }
